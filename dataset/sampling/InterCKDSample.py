...
class InterCKDSample(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for i in sample_indices:
            img, lbl, logit_t, input_cached = self.original_dataset[i]
            samples.append(img.unsqueeze(0))
            teacher_logits.append(logit_t.unsqueeze(0))
            cached_inputs.append(input_cached.unsqueeze(0))
            labels.append(lbl)
        pos = torch.cat(samples[:self.pos])
        neg = torch.cat(samples[self.pos:])
        cached_inputs_pos = torch.cat(cached_inputs[:self.pos])
        cached_inputs_neg = torch.cat(cached_inputs[self.pos:])
        ...
        return pos, neg, label_pos, label_neg, teacher_logits_pos, teacher_logits_neg, cached_inputs_pos, cached_inputs_neg