...
"""from https://github.com/lenscloth/RKD/blob/master/metric/batchsampler.py"""

def index_dataset(dataset: ImageFolder):
    kv = [(cls_ind, idx) for idx, (_, cls_ind, _) in enumerate(dataset)]
    cls_to_ind = {}
...
class RKDSampler(Sampler):
    def __init__(self, data_source: ImageFolder, batch_size, m=5, iter_per_epoch=200):
        ...
        self.class_idx = set()
        for _, cls, _ in data_source:
            self.class_idx.add(cls)
        self.class_idx = list(self.class_idx)
